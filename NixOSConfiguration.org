...
- [[#desktop-environment][Desktop Environment]]
  ...
  - [[#kde-plasma5--xmonad][KDE Plasma5 + XMONAD]]
...
- [[#plex-media-server][Plex Media Server]]
  - [[#nix-plex-services][Nix Plex Services]]
...
* Systemd Timers
See https://nixos.wiki/wiki/Systemd/Timers for details on configuring timers
This timer will send a request every 5 minutes to duckdns to update my current ip
#+BEGIN_SRC nix :tangle no :noweb-ref insert-config-here
...
NOTE not in used (notice its not being tangled) using systemd timers instead
#+BEGIN_SRC nix :tangle no
services.cron = {
  enable = true;
  systemCronJobs = [
...
* Nginx
See [[https://nixos.wiki/wiki/Nginx][NixOS nginx configuration]] for information on how to use configure NGINX declaratively within NixOS.
#+BEGIN_SRC nix :tangle no :noweb-ref insert-config-here
...
* Plex Media Server
[[https://nixos.wiki/wiki/Plex][Plex]] allows you to aggregate all your personal media and access it anywhere.
Go to http://127.0.0.1:32400/web/ to configure
** Nix Plex Services
#+BEGIN_SRC nix :tangle no 
services.plex = {
  enable = true;
...
[[https://nixos.wiki/wiki/Jellyfin][Jellyfin]] allows you to aggregate all your personal media and access it anywhere.
Go to http://localhost:8097 to configure
#+BEGIN_SRC nix :tangle no :noweb-ref insert-config-here
services.jellyfin = {
  enable = true;
...
** Path Variable
Add extra directories to your PATH here
